...
/*
	...
	위 사이트에 나와 있는 6개의 라이브러리 외 나머지 라이브러리들은 의존성 문제 때문에 직접 추가적으로 등록시켜준 것이다.(apache-commons-lang.jar 제외)
	※Maven을 사용할 경우에는 의존성 전이가 되므로 위 사이트에 나와 있는 대로 6개의 라이브러리에 대한 dependency 설정만 추가해주면 된다.
